...
def test_v4_reaches_full_context_and_trains():
    """The dilation ladder must give position T-1 a path from position 0
    (perturb token 0, the last prediction moves), and the stack must train
    with tied weights like its siblings."""
    V, C, T = 256, 16, 32
    net = _make_v2(V, C, T, arch='v4')
    assert net.head[-1].weight is net.token_embedding_table.weight
    net.eval()
    ...
    with torch.no_grad():
        ya = net(a).reshape(T, V)
        yb = net(b).reshape(T, V)
    assert not torch.allclose(ya[-1], yb[-1]), \
        "token 0 has no path to the last prediction"
